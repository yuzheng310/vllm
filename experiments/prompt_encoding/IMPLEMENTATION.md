...
服务接入发现原生 API 在工具定义进入模板前执行 `tool.model_dump()`，
会重排 JSON 字段。直接 HF 的旧冻结 token IDs 因而不能未经核查就作为
HTTP 期望值；首个 native 请求在缓存关闭时已不匹配，比较当即停止。
服务对照单独按该原生转换冻结期望 IDs，原始 233 份 messages/tools 请求
体完全不变，并断言工具内容在字典意义上完全相同。新参考必须先通过
完整 native A1 的 HTTP 精确比较，才允许候选 B1 执行；旧参考与失败记录
全部保留。最终指标必须区分直接分词和实际 API 的两个输入契约。

该 base 的 render 退出流程引用 None engine_client，关闭会挂起，因此
...
fastokens 0.3.1 原生不跟踪字符 offsets，关闭缓存的 API 已返回 501。
该后端的 offsets 检查改为验证开启/关闭缓存均返回相同 501 和原生错误，
HF 仍必须返回完整 offsets。这个兼容性检查发生在性能主回放之后；不得
宣称 fastokens 支持 offsets，也不得把原生限制当作缓存新增的回归。
