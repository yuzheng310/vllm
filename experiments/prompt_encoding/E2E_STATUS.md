...
随后 09:10:05 UTC 的只读查询再次确认所有控制日志和输出目录都不存在，
GPU 计算进程为空。只有完成该核对后才再次提交，并增加控制日志防覆盖条件、
放宽 SSH 的短暂断连容忍时间。09:11:57 UTC 前收到 **controller_pid=1444656**，
首次明确确认控制器启动。它随后是否通过资源预检、加载 GPU 或完成回放，
仍须由状态文件确认，不能只凭 PID 判断。

09:18:41 UTC 前，后续 SSH 监控和 IPv4 文件传输均再次超时。没有重复启动，
没有将 PID 当成成功实验。恢复连接后先检查目录、日志和进程，不覆盖已有日志。
...
本轮尚无可报告的端到端提升百分比。此前每请求 0.52 ms 节省仍只是 CPU
